...
/*made this fonction to test passing by pointer */
void f(int* n)
{
	printf("n = %d \n",*n);
	(*n)++;
...
{
	printf("this lecture has this part and it talks about passing by value or by pointer. \n" );
	int a = 5;
	printf("a = %d \n",a);
	f(&a);
  ...
  printf("----------------------------------------------------------------------\n\n\n");
	...
	/* also here made some few changes to have an averisment if i had abug with intarray_get_index_of_min_from */

	int init_index = 65;
	n = intarray_get_index_of_min_from(tutu,init_index);
	printf("the index of minimum value of tutu table from %d is :%d\n",init_index, n);
	printf("le contenu de la case est : %d\n", tutu.data[n]);
